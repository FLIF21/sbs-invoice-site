...
- кнопка оплаты, безопасный тестовый сценарий и готовая интеграция ЮKassa с подтверждением через webhook;
...
| `PAYMENT_PROVIDER` | `disabled`, локальный `test` или `yookassa` |
| `YOOKASSA_SHOP_ID` | идентификатор корпоративного магазина ЮKassa |
| `YOOKASSA_SECRET_KEY` | секретный серверный ключ ЮKassa; не передавать в браузер и не хранить в Git |
| `YOOKASSA_TEST_MODE` | `true` только для тестового магазина ЮKassa |
...
Тестовая оплата доступна только вне production и не списывает деньги. Для боевого запуска задайте `PAYMENT_PROVIDER=yookassa`, корпоративные ключи и зарегистрируйте в ЮKassa URL уведомлений `https://ваш-домен/api/payments/yookassa`. Статус счёта меняется на «Оплачен» только после повторной серверной проверки платежа по API ЮKassa.
